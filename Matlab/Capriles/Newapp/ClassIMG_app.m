classdef ClassIMG_app < handle
    ...
    methods
        ...
        function Image(obj)
            ...
            X1_A = Image_App.xce.Value; Y1_A =  str2double(get(app.yce,'string'));
            DX_A = str2double(get(app.dex,'string')); DY_A =  str2double(get(app.dey,'string'));
            N_A1 = str2double(get(app.dot,'string')); N_A = int64(N_A1);
            tpause1_A = str2double(get(app.tef,'string'));
            Tiempo_int = str2double(get(app.tin,'string'));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            function input(x,y,vec1,vec2,paso)
                ...
                h = pcolor(app.axes10,8*datax,8*datax,vec1); set(h, 'EdgeColor', 'none');
                colorbar(app.axes10)
                colormap(app.axes10,'jet')
                plot(app.axes11,8*datax,vec2)
                a=vec1; b=vec2;
                set(app.cux,'String',num2str(round(8*datax(x),3)));
                set(app.cuy,'String',num2str(round(8*datay(y),3)));
                set(app.cnt,'String', round(cnt));
                set(app.tiempo_integracion,'String', 1000*Int_time);
                set(app.scn,'String', round(std_A_zeros));
                set(app.stp,'String', paso*1000);
            end
        ...
        function Trace(obj)
            ...
            mouse = Robot;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            CurrentX = str2double(get(app.cux,'string'));
            CurrentY = str2double(get(app.cuy,'string'));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            X1_A = str2double(get(app.xce,'string')); Y1_A =  str2double(get(app.yce,'string'));
            DX_A = str2double(get(app.dex,'string')); DY_A =  str2double(get(app.dey,'string'));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            cnt=0;
            arct=1;
            ...
            while and(arct==1, Gstop==1)
                N_A1 = str2double(get(app.dot,'string'));
                N_A = int64(N_A1);
                Tiempo_int = str2double(get(app.tin,'string'));
                ...
                if row >= Trace_lenght
                    cntaverage = mean(cnt);
                    cntstd = std(cnt);
                    set(app.meancounts,'string',round(cntaverage));
                    set(app.stdcounts,'string',round(cntstd));
                end
                set(app.tiempo_integracion,'String', 1000*Int_time);
                plot(app.axes12,cnt(1:Trace_lenght));
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                
                cumouse = get(app.axes10, 'CurrentPoint');
                Xcupos = cumouse(1);
                Ycupos = cumouse (3);
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                set(app.cux,'String',num2str(round(8*xcen,3)));
                set(app.cuy,'String',num2str(round(8*ycen,3)));
                drawnow;
                if not(Gstop); Gstop = 2; obj.StopProgram(); end
            end
        end
        ...
        function StopProgram (obj)
            global Gstop
            if Gstop == 2
                CurrentX = str2double(get(app.cux,'string'))/8;
                CurrentY = str2double(get(app.cuy,'string'))/8;
                ...
                for i = 1:max(size(datax))
                    ...
                    set(app.cux,'String',num2str(round(8*datax(i),3)));
                    set(app.cuy,'String',num2str(round(8*datay(i),3)));
                end
                clc
                clear all
                close all
